...
const sentMsg = (txt) =>
{
    return `<div class="sent-msg-container">
                <p class="sent-msg">${txt}</p>
            </div>`
}

const sentImg = (img) => 
{
    return `<div class="img-container sent-img">
                ${img}
            </div>`
}
...
{
    return `<div class="received-msg-container">
...
form.addEventListener("submit", e => 
{
    ...
    msgs.innerHTML += sentMsg(input.value)
    if (img) msgs.innerHTML += sentImg(img.outerHTML)
...
socket.on("message:receive", payload => {
    let now = new Date().toLocaleTimeString('it-IT').slice(0, 5)
    msgs.innerHTML += receivedMsg(payload.message)
